 ...
 * @author datafox
 */
public class MultiplyOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SingleParameterOperation) Operations::multiply);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("mulOp", MultiplyOperationSourceNodeDefinition.class);
    }
}
