 ...
 * @author datafox
 */
public class CbrtOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SourceOperation) Operations::cbrt);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("cbrtOp", CbrtOperationSourceNodeDefinition.class);
    }
}
