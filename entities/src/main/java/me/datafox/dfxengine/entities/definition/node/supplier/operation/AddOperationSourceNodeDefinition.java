 ...
 * @author datafox
 */
public class AddOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SingleParameterOperation) Operations::add);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("addOp", AddOperationSourceNodeDefinition.class);
    }
}
