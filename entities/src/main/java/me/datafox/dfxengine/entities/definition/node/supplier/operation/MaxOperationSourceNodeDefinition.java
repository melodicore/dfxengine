 ...
 * @author datafox
 */
public class MaxOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SingleParameterOperation) Operations::max);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("maxOp", MaxOperationSourceNodeDefinition.class);
    }
}
