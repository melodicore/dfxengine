 ...
 * @author datafox
 */
public class InverseOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SourceOperation) Operations::inverse);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("invOp", InverseOperationSourceNodeDefinition.class);
    }
}
