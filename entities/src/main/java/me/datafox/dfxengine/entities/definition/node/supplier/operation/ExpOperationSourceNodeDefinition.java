 ...
 * @author datafox
 */
public class ExpOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SourceOperation) Operations::exp);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("expOp", ExpOperationSourceNodeDefinition.class);
    }
}
