 ...
 * @author datafox
 */
public class SubtractReversedOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                Modifiers.reverse(Operations::subtract));
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("subROp", SubtractReversedOperationSourceNodeDefinition.class);
    }
}
