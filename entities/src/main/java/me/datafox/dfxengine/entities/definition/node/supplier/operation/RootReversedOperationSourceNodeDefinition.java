 ...
 * @author datafox
 */
public class RootReversedOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                Modifiers.reverse(Operations::root));
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("rootROp", RootReversedOperationSourceNodeDefinition.class);
    }
}
