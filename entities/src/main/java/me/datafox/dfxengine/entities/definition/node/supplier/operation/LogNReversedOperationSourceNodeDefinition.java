 ...
 * @author datafox
 */
public class LogNReversedOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                Modifiers.reverse(Operations::logN));
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("logNROp", LogNReversedOperationSourceNodeDefinition.class);
    }
}
