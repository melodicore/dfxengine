 ...
 * @author datafox
 */
public class SubtractOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SingleParameterOperation) Operations::subtract);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("subOp", SubtractOperationSourceNodeDefinition.class);
    }
}
