 ...
 * @author datafox
 */
public class LogNOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SingleParameterOperation) Operations::logN);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("logNOp", LogNOperationSourceNodeDefinition.class);
    }
}
