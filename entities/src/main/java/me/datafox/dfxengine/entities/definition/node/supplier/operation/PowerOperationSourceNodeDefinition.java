 ...
 * @author datafox
 */
public class PowerOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SingleParameterOperation) Operations::power);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("powOp", PowerOperationSourceNodeDefinition.class);
    }
}
