 ...
 * @author datafox
 */
public class LogOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SourceOperation) Operations::log);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("logOp", LogOperationSourceNodeDefinition.class);
    }
}
