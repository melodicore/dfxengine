 ...
 * @author datafox
 */
public class MinOperationSourceNodeDefinition implements SupplierNodeDefinition<SourceNode<Operation>> {
    @Override
    public SourceNode<Operation> build(NodeTree tree, Context context) {
        return new SourceNode<>(tree, SingleDataTypeImpl.of(Operation.class),
                (SingleParameterOperation) Operations::min);
    }

    @Component
    public static ClassTag getTag() {
        return new ClassTag("minOp", MinOperationSourceNodeDefinition.class);
    }
}
